...
import com.mantum.cmms.service.ProveedorServices;
...
public class ProveedorAutocompleteActivity extends Mantum.Activity {

    private ProgressBar progressBar;

    public static final int REQUEST_ACTION = 1221;

    private static final String TAG = ProveedorAutocompleteActivity.class.getSimpleName();

    private  ProveedorServices proveedorService;

    private AlphabetAdapter<Proveedor> alphabetAdapter;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        try {
            ...
            includeBackButtonAndTitle(R.string.filtro_proveedor_autocomplete);
            setContentView(R.layout.autocomplete_proveedor);
            ...
            proveedorService =  new ProveedorServices(this, cuenta);
            alphabetAdapter = new AlphabetAdapter<>(this);

            isEmptyList();
            alphabetAdapter.setOnAction(new OnSelected<Proveedor>() {

                @Override
                public void onClick(final Proveedor value, int position) {
                ...
                @Override
                public boolean onLongClick(Proveedor value, int position) {
                    return true;
                }
            });
    ...
    private void onNext(Response response) {
        ...
        List<Proveedor> listaProveedor = response.getBody(Proveedor.Request.class)
                .getProveedor();

        if (listaProveedor.isEmpty()){
            Snackbar.make(getView(), R.string.proveedor_autocomplete_empty, Snackbar.LENGTH_LONG)
                    .show();
            return;
        }

        for (Proveedor proveedor : listaProveedor){
            if (proveedor.getCedula() != null)  alphabetAdapter.add(proveedor);
        }
    ...
    private void request(String query) {
        ...
        compositeDisposable.add(proveedorService.get(query)
